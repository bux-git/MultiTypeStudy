package com.dqr.www.multitypestudy.widget.banner;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private int mIndicatorSelectedResId = R.drawable.point_gray_radius;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.point_gray_radius);
    ...
    private void setImageArray(Object[] imagesUrl, OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageListener != null) {
                ...
                if (defaultImage != -1)
                    Glide.with(context).load(url).crossFade().placeholder(defaultImage).fitCenter().into(iv);
                else
                    Glide.with(context).load(url).crossFade().fitCenter().into(iv);
            }
        }
        setData();
    }
...
//        Log.i(tag, ev.getAction() + "--" + isAutoPlay);
        if (isAutoPlay) {
            ...
            if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL
                    || action == MotionEvent.ACTION_OUTSIDE ||action == MotionEvent.ACTION_MOVE ) {
//                startAutoPlay();
            } else if (action == MotionEvent.ACTION_DOWN ) {
//                stopAutoPlay();   //
//                ToastUtil.showShort("暂停了");
            }
        }
