...
public class XListViewFooter extends LinearLayout {
    ...
    public final static int STATE__NO_MORE = 3;
    ...
    public void onMove(float delta) {
            ...
            if (getState() != 3 && mEnablePullLoad && getState() != 2) {

                if (height > mMeasuredHeight / 2) { // height enough to invoke load
                    setState(XListViewFooter.STATE_READY);
                } else {
                    setState(XListViewFooter.STATE_NORMAL);
                }
            }
    ...
    public boolean releaseAction() {
        ...
        int height = getBottomMargin();
        if (height == 0) // not visible.
            isOnRefresh = false;

        if (getBottomMargin() > mMeasuredHeight / 2&&getState() != 3) {
            setState(XListViewFooter.STATE_READY);
            isOnRefresh = true;
        }
    ...
    private void smoothScrollTo() {
        ValueAnimator animator = ValueAnimator.ofInt(getBottomMargin(), 0);
